...
public class CropImageActivity extends AppCompatActivity
    ...
    private void startWithCompat() {
        if (mOptions == null) return;

        if (mOptions.cropSourceFlag == 1) {
            CropImage.startPickImageDirectly(this);
        } else if (mOptions.cropSourceFlag == 2) {
            Intent captureIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            File imageFile = createImageFile(this);
            if (imageFile != null) {
                mPendingFilePath = imageFile.getAbsolutePath();
                Uri contentUri = FileProvider.getUriForFile(this, getPackageName(), imageFile);
                if (contentUri != null) {
                    captureIntent.putExtra(MediaStore.EXTRA_OUTPUT, contentUri);
                    startActivityForResult(captureIntent, PICK_IMAGE_CHOOSER_REQUEST_CODE);
                }
            }
        } else {
        ...
        if (requestCode == PICK_IMAGE_CHOOSER_REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (CropImage.isReadExternalStoragePermissionsRequired(this, mCropImageUri)) {
                    ...
                    // no permissions required or already grunted, can start crop image activity
                    if (mOptions.cropSourceFlag == 2) {
                        if (mPendingFilePath != null) {
                            Uri uri = Uri.fromFile(new File(mPendingFilePath));
